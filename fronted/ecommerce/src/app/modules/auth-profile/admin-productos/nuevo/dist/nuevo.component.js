"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
exports.__esModule = true;
exports.NuevoComponent = void 0;
var core_1 = require("@angular/core");
var Producto_1 = require("../../_model/Producto");
var NuevoComponent = /** @class */ (function () {
    function NuevoComponent(router, authService) {
        this.router = router;
        this.authService = authService;
        this.modelProducto = new Producto_1.Producto();
    }
    NuevoComponent.prototype.ngOnInit = function () {
    };
    NuevoComponent.prototype.Guardar = function (producto) {
        var _this = this;
        this.authService.createProducto(producto)
            .subscribe(function (data) {
            alert('Se agrego con exito');
            _this.router.navigate(['/auth/admin-productos/listar']);
        });
    };
    NuevoComponent = __decorate([
        core_1.Component({
            selector: 'app-nuevo',
            templateUrl: './nuevo.component.html',
            styleUrls: ['./nuevo.component.css']
        })
    ], NuevoComponent);
    return NuevoComponent;
}());
exports.NuevoComponent = NuevoComponent;
